                        ...
                        <div class="form-group projects-wrapper<?php echo e($hide_project_selector); ?>">
                         ...
                         echo '<option value="' . $expense->project_id . '" selected>' . e(get_project_name_by_id($expense->project_id)) . '</option>';
                            ...
                            */ ?>
                            ...
                            $item_selected = (isset($expense) && $expense->item_list != 0) ? $expense->item_list : '';
                            echo render_select('item_list', [], ['id', 'description'], 'Item List', $item_selected);
                            ?>
                        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        var selected_item = '<?php echo $item_selected; ?>';
    ...
    function load_custom_field_with_selected(project_id, selected_value) {
        var customSelect = $('select[name="item_list"]');

        $.get(admin_url + 'expenses/get_invoice_items/' + project_id, function(response) {
            response = JSON.parse(response);
            customSelect.html('');
            customSelect.append('<option value=""></option>');

            $.each(response, function(i, item) {
                var selected = (item.id == selected_value) ? 'selected' : '';
                customSelect.append('<option value="' + item.id + '" ' + selected + '>' + item.description + '</option>');
            });
